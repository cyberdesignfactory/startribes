...
;; This is the original version (generated by Gemini)
;; (re-frame/reg-event-fx
;;  ::request-orientation-permission
;;  (fn [_ _]
;;    ;; Feature detect the permission API (iOS 13+)
;;    (if (and (exists? js/DeviceOrientationEvent)
;;             (fn? js/DeviceOrientationEvent.requestPermission))
;;      (.then (js/DeviceOrientationEvent.requestPermission)
;;             (fn [response]
;;               (if (= response "granted")
 ...
 (fn [_ _]
   (.addEventListener js/window "deviceorientation"
                      (fn [event]
                        ...
                        #_(re-frame/dispatch [::set-orientation-data 
                                            {:alpha (.-alpha event) 
                                             :beta  (.-beta event) 
                                             :gamma (.-gamma event)}])))
...
(re-frame/reg-event-db
 ...
 (fn [db _]
   ...
   #_(if (.-requestPermission js/DeviceOrientationEvent)

     (let [!resp (.requestPermission js/DeviceOrientationEvent)]

       (js/alert (str @!resp))

       (if (= "granted" @!resp)
         #_(re-frame/dispatch [::initialize-orientation-listener])
         (js/alert "Orientation permission IS granted")
         (js/alert "Orientation permission not granted")

         )
       )

     (re-frame/dispatch [::initialize-orientation-listener]))
